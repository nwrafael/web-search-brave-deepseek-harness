/**
 * The Brave search card's staged form over the `web-search-brave` settings
 * namespace.
 *
...
/**
 * Namespace of the Brave search provider. Spelled here rather than
 * imported: a client package must not depend on a Host package.
 */
export const BRAVE_SEARCH_NS = 'web-search-brave'
...
export interface BraveSearchSettings {
  ...
  baseURL?: string
  /** Default result count sent when a search carries no bound of its own. */
  numResults?: number
  /** Two-letter country bias sent as `country`. */
  country?: string
  /** Search language sent as `search_lang` (for example `pt`, `en`). */
  searchLang?: string
  /** SafeSearch mode (`off`, `moderate`, `strict`). */
  safeSearch?: string
  /** Recency window (`pd`, `pw`, `pm`, `py`). */
  freshness?: string
}
...
/** What the Brave search card renders. */
export interface BraveSearchCardState extends CardShell {
  /** Provider endpoint. */
  ...
  /** Search language. */
  searchLang: CardFieldState
  /** SafeSearch mode. */
  safeSearch: CardFieldState
  /** Recency window. */
  freshness: CardFieldState
...
/** The registration-side face the Brave search card's slot entry injects. */
export interface BraveSearchCardFace extends CardActions {
  hooks: {
    /** Card snapshot bound by the renderer as useBraveSearchCard. */
    braveSearchCard: SnapshotStore<BraveSearchCardState>
  }
}

/** Bridges the `web-search-brave` scope and the credentials domain onto the card. */
export class BraveSearchCardController {
  private readonly form: CardForm<BraveSearchSettings>
  private readonly store: SnapshotStore<BraveSearchCardState>
  private credential: CredentialState = { ref: '', configured: false, writable: true }

  /**
   * @param scope - the bound settings scope for the `web-search-brave` namespace.
   * @param api - wire face used for the credential the section references.
   */
  constructor(
    private readonly scope: SettingsScope<BraveSearchSettings>,
    private readonly api: Pick<IApiClient, 'credentials'>,
  ) {
    this.form = new CardForm(
      scope,
      [
        textField('baseURL'), numberField('numResults'), textField('country'),
        textField('searchLang'), textField('safeSearch'), textField('freshness'),
      ],
  ...
  private projection(): BraveSearchCardState {
    return {
      ...this.form.shell(),
   ...
   */
  inject(): BraveSearchCardFace {
    return { hooks: { braveSearchCard: this.store }, ...this.form.actions() }
  }
 ...
 */
function refOf(snapshot: SettingsScopeSnapshot<BraveSearchSettings>): string {
